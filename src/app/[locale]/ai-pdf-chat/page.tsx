...
export default function AiPdfChatPage() {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    if (acceptedFiles.length > 0) {
      ...
      try {
        ...
        // Add welcome message
        setChatMessages([
          { role: 'model', text: `Bonjour ! J'ai lu votre document **${selected.name}** (${count || 1} pages). De quoi souhaitez-vous discuter aujourd'hui ? Posez-moi vos questions !` }
        ]);
      } catch (err: any) {
  ...
  const workspaceSidebar = (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div className="workspace-sidebar-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.2rem' }}>
        <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '1.1rem', fontWeight: 'bold' }}>
          <MessageSquare size={20} color={ACCENT} /> Discussion PDF
        </h3>
      ...
      <div className="workspace-sidebar-content" style={{ flex: 1, overflowY: 'auto', padding: '1.2rem', display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
  ...
  return (
    <ToolLayout
      ...
      processingLabel="Indexation du PDF en cours..."
      successMessage=""
      seoSection={
