...
import { ImageIcon, FilePlus, CheckCircle2 } from 'lucide-react';
import { ToolLayout, useToolState } from '@/components/tools';

const TOOL_COLOR = '#eab308'; // Yellow for JPG
...
export default function PdfToJpgPage() {
  ...
  const workspacePreview = file && (
    <div className="pdf-page-card" style={{ width: 300, height: 420, position: 'relative', overflow: 'hidden', boxShadow: '0 20px 50px rgba(0,0,0,0.15)', border: '1px solid var(--glass-border)' }}>
      ...
      <div className="pdf-page-content" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', flexDirection: 'column', gap: 8 }}>
        <ImageIcon size={60} color={TOOL_COLOR} style={{ opacity: 0.5 }} />
        <span style={{ fontSize: '0.9rem', color: 'var(--color-text-secondary)' }}>Convertir en JPG</span>
      </div>
    </div>
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      >
        <div style={{ padding: 8, background: `${TOOL_COLOR}15`, borderRadius: 8 }}>
          <ImageIcon size={24} color={TOOL_COLOR} />
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: TOOL_COLOR, textTransform: 'uppercase', marginBottom: 4 }}>PAGE EN JPG</div>
  ...
  return (
    <ToolLayout
      ...
      successSubtitle="Le PDF a été converti en images JPG avec succès."
      ...
      seoSection={
        <div style={{ margin: '4rem auto', textAlign: 'center', maxWidth: '800px' }}>
          <h2 style={{ fontSize: '2rem', marginBottom: '1.5rem' }}>Convertir PDF en JPG gratuit et sécurisé</h2>
