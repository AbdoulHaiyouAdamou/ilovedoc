...
export async function generateAiReportPdf(
  ...
  page.drawText(cleanForPdf(title), {
    x: 30,
    y: height - 45,
    size: 16,
    font: helveticaBold,
  ...
  page.drawText(cleanForPdf(subtitle), {
    x: 30,
    y: height - 65,
    size: 9,
    font: helveticaFont,
  ...
  page.drawText(cleanForPdf(metaText), {
    x: 30,
    y,
    size: 10,
    font: helveticaBold,
  ...
  for (let line of lines) {
    ...
    if (line.startsWith('#')) {
      ...
      const text = line.replace(/^[-*]\s*/, '');
      y -= 15;
      page.drawCircle({ x: 38, y: y + 3, size: 2, color: rgb(r, g, b) });
  ...
  page.drawText(cleanForPdf('Document genere via iLoveDoc - 100% gratuit.'), {
    x: 180,
    y: 15,
    size: 7,
    font: helveticaFont,
